...
{
    return (center_.down_by(radius_).y < 0);
}

bool Missile::hits_bottom(Geometry const& geometry) const
{
    return (center_.down_by(radius_).y > 1024);
}
...
int Missile::hits_missile(std::vector<Missile>& list)
{
    int flag = 0;
    for(Missile& obj : list){
        Missile next = obj.next();
        int dist = (obj.center_.x - center_.x)*(obj.center_.x - center_.x) +
                   (obj.center_.y - center_.y) * (obj.center_.y - center_.y);
        if(dist < 4*radius_*radius_){
            if(*this != obj){
                if(is_enemy && obj.is_enemy){
                    obj.missile_bounce_();
                    //std::cout<<"and bounces at"<<next.top_left().x<<" ,
                    // "<<next.top_left().y<<" at distance of"<<dist<<"\n";
                    if(flag !=2) flag = 1; //bounce
...
void Missile::missile_bounce_(){
    this->velocity_.width = this->velocity_.width * (-1);
    if(this->velocity_.width>0) this->velocity_.width+1;
    if(this->velocity_.width<0) this->velocity_.width-1;
    if(this->velocity_.width == 0)  this->velocity_.width + (rand()%2 - 1);
}
